...
from nanochat.dataset import set_dataset
from nanochat.common import compute_init, compute_cleanup, print0, DummyWandb, print_banner, get_base_dir, autodetect_device_type, get_peak_flops
from nanochat.tokenizer import get_tokenizer, get_token_bytes
from nanochat.checkpoint_manager import save_checkpoint, load_checkpoint
from nanochat.loss_eval import evaluate_bpb, evaluate_bpb_sequential
...
parser.add_argument("--dataset", type=str, default="fineweb", choices=["fineweb", "climbmix"], help="pretraining dataset (download it first via `python -m nanochat.dataset -d <name>`)")
# Model architecture
...
parser.add_argument("--no-value-embeds", action="store_true", help="disable value embeddings")
...
parser.add_argument("--no-save", action="store_true", help="skip ALL checkpoint saving (overrides --save-every and the end-of-run save). Useful for quick ablation runs where you only care about logged metrics.")
# Output
...
print0(f"use_value_embeds: {not args.no_value_embeds}")
print0(f"use_x0_residual: {not args.no_x0_residual}")
print0(f"use_resid_lambdas: {not args.no_resid_lambdas}")
...
if args.compile_interleaved:
    # Compile the interleaved-pass forward as well (by default it runs eagerly since
    # forward_interleaved_train invokes self.forward_interleaved on the uncompiled module).
    orig_model._compiled_forward_interleaved = torch.compile(orig_model.forward_interleaved, dynamic=args.compile_dynamic)
...
num_scaling_params = param_counts['transformer_matrices'] + param_counts['lm_head'] + param_counts['interleaved_matrices'] # determined to give the cleanest scaling laws, see dev/LOG.md Jan 27, 2026
...
while True:
    ...
    # Accumulate all eval log data for this step, then log once to avoid shifting wandb's internal step counter
    eval_log_data = None
    ...
    if args.eval_every > 0 and (last_step or step % args.eval_every == 0):
        ...
        if per_pass_bpb is not None:
            for lbl, bpb in zip(pass_labels, per_pass_bpb):
    ...
    if args.interleaved and args.eval_seq_every > 0 and (last_step or (step > 0 and step % args.eval_seq_every == 0)):
        ...
        if args.eval_seq_full_pass_sanity:
            seq_val_loader_sanity = tokenizing_distributed_data_loader_bos_bestfit(tokenizer, args.eval_seq_batch_size, args.max_seq_len, split="val", device=device)
            with autocast_ctx:
                sanity_result = evaluate_bpb_sequential(orig_model, seq_val_loader_sanity, seq_eval_steps, token_bytes, mode='full_pass_sanity')
            val_bpb_seq_sanity = sanity_result['bpb']
            print0(f"Step {step:05d} | Validation bpb_seq(full_pass_sanity): {val_bpb_seq_sanity:.6f} (should ≈ parallel full pass)")
            if sanity_result['bucket_bpb']:
                bucket_str = " | ".join([f"{s}-{e}: {bpb:.6f}" for s, e, bpb in sanity_result['bucket_bpb']])
                print0(f"  bpb_seq(full_pass_sanity) by position: {bucket_str}")
        # Recurrent mode: feed h_final from previous position
        seq_val_loader = tokenizing_distributed_data_loader_bos_bestfit(tokenizer, args.eval_seq_batch_size, args.max_seq_len, split="val", device=device)
        with autocast_ctx:
            recurrent_result = evaluate_bpb_sequential(orig_model, seq_val_loader, seq_eval_steps, token_bytes, mode='recurrent')
        val_bpb_seq = recurrent_result['bpb']
        print0(f"Step {step:05d} | Validation bpb_seq(recurrent): {val_bpb_seq:.6f}")
        if recurrent_result['bucket_bpb']:
            bucket_str = " | ".join([f"{s}-{e}: {bpb:.6f}" for s, e, bpb in recurrent_result['bucket_bpb']])
            print0(f"  bpb_seq(recurrent) by position: {bucket_str}")
        if eval_log_data is None:
            eval_log_data = {"step": step, "total_training_flops": flops_so_far, "total_training_time": total_training_time}
        eval_log_data["val/bpb_seq"] = val_bpb_seq
        if args.eval_seq_full_pass_sanity:
            eval_log_data["val/bpb_seq_sanity"] = val_bpb_seq_sanity
        model.train()

    # Single wandb log call for all bpb/bpb_seq data this step
    ...
    if args.interleaved and args.eval_every > 0 and (last_step or step % args.eval_every == 0):
        ...
        with autocast_ctx:
            orig_model.print_interleaved_diagnostics(batches=diag_loader, n_steps=4)
        model.train()
    ...
    # --no-save disables both paths entirely (useful for cheap ablation runs).
