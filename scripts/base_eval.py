...
def main():
    ...
    parser.add_argument('--eval-seq-batch-size', type=int, default=32, help='Per-device batch size for sequential BPB eval (larger = faster, limited by KV cache memory)')
    parser.add_argument('--split-tokens', type=int, default=40*524288, help='Number of tokens to evaluate per split for BPB')
    parser.add_argument('--device-type', type=str, default='', help='cuda|cpu|mps (empty = autodetect)')
    parser.add_argument('--dataset', type=str, default='fineweb', choices=['fineweb', 'climbmix'], help='pretraining dataset used for the BPB evaluation splits')
    args = parser.parse_args()
    set_dataset(args.dataset)

    # Parse evaluation modes
    eval_modes = set(mode.strip() for mode in args.eval.split(','))
    valid_modes = {'core', 'bpb', 'bpb_seq', 'sample'}
    ...
    if 'bpb_seq' in eval_modes:
        ...
        if not use_interleaved:
            ...
            print0("="*80)
            seq_bs = args.eval_seq_batch_size
            tokens_per_step = seq_bs * sequence_len * ddp_world_size
            split_tokens = args.split_tokens
            if split_tokens % tokens_per_step != 0:
                split_tokens = (split_tokens // tokens_per_step) * tokens_per_step
                print0(f"Adjusted split_tokens to {split_tokens} for seq eval (must be divisible by {tokens_per_step})")
            seq_steps = split_tokens // tokens_per_step
            loader = tokenizing_distributed_data_loader_bos_bestfit(tokenizer, seq_bs, sequence_len, "val", device=device)
            with autocast_ctx:
                seq_result = evaluate_bpb_sequential(model, loader, seq_steps, token_bytes)
            bpb_seq = seq_result['bpb']
            bpb_results['val_seq'] = bpb_seq
            print0(f"val bpb_seq: {bpb_seq:.6f}")
            if seq_result['bucket_bpb']:
                bucket_str = " | ".join([f"{s}-{e}: {bpb:.6f}" for s, e, bpb in seq_result['bucket_bpb']])
                print0(f"  bpb_seq by position: {bucket_str}")
    ...
    if bpb_results:
        ...
        if "val_seq" in bpb_results:
            report_data[0]["val bpb_seq"] = bpb_results["val_seq"]
