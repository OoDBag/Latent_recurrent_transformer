...
class DistMuonAdamW(torch.optim.Optimizer):
    ...
    def _reduce_adamw(self, group: dict, world_size: int) -> dict:
        ...
        for p in group['params']:
            grad = p.grad
            # reduce_scatter requires shape[0] to be divisible by world_size; fall back to
            # all_reduce when it isn't (e.g., a stacked scalar param with a small leading dim
            # under world_size=8). The all_reduce path also handles small params.
            if p.numel() < 1024 or p.shape[0] % world_size != 0:
                # Small / non-shardable params: all_reduce (no scatter/gather needed)
